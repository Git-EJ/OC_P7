...
const results = new Array(50)
const searchFilterLoop = (searchText) => {
  if (searchText.length < 3) return cards.cards
  const search = searchText.toLowerCase()
  if (cards.cards.length > results.length) { results.length = cards.cards.length }
  let x = 0
  ...
  for (const card of cards.cards) {
    if (card.name.toLowerCase().includes(search) || card.description.toLowerCase().includes(search)) {
      ...
      for (const cardIngredients of card.ingredients) {
        if (cardIngredients.ingredient.toLowerCase().includes(search)) {
          results[x++] = card
          break
        }
      }
